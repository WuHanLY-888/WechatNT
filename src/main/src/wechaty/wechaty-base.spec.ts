...
test('wechatifyUserModules init()', async (t) => {
    ...
    t.doesNotThrow(
        () => wechatyTest.init(),
        'should not throw for the 2nd time init (silence skip)'
    )
})
...
test('WechatySkeleton: super.{start,stop}()', async (t) => {
    ...
    t.ok(
        ...
        'should call the skeleton start(), which means all mixin start()s are chained correctly'
    )
    ...
    t.ok(
        stopStub.calledOnce,
        'should call the skeleton stop(), which means all mixin stops()s are chained correctly'
    )
})
