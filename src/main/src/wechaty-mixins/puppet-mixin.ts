 ...
 */
const puppetMixin = <MixinBase extends WechatifyUserModuleMixin & GErrorMixin & IoMixin>(
    mixinBase: MixinBase
) => {
    ...
    abstract class PuppetMixin extends mixinBase {
        ...
        override async init(): Promise<void> {
            ...
            if (this.__puppetMixinInited) {
                log.verbose(
                    'WechatyPuppetMixin',
                    'init() skipped because this puppet has already been inited before.'
                )
                return
            }
            ...
            const puppetInstance = await PUPPET.helpers.resolvePuppet({
                ...
                puppetOptions:
                    'puppetOptions' in this.__options ? this.__options.puppetOptions : undefined
            })
                ...
                switch (eventName) {
                    ...
                    case 'error':
                        puppet.on('error', (payload) => {
                            ...
                            this.emit(
                                'error',
                                GError.from(payload.gerror || payload.data || payload)
                            )
                        })
                        break
                    ...
                    case 'login':
                        puppet.on('login', async (payload) => {
                            try {
                                const contact = await this.ContactSelf.find({
                            ...
                            try {
                                ...
                                if (!msg) {
                                    this.emit(
                                        'error',
                                        GError.from(
                                            'message not found for id: ' + payload.messageId
                                        )
                                    )
                                    return
                                }
                                ...
                                if (room) {
                                    ...
                                    this.emit(
                                        'error',
                                        GError.from('message without room and listener')
                                    )
                                }
                            } catch (e) {
                                this.emit('error', GError.from(e))
                        ...
                        puppet.on('post', async (payload) => {
                            try {
                                ...
                                if (!post) {
                                    this.emit(
                                        'error',
                                        GError.from('post not found for id: ' + payload.postId)
                                    )
                                    return
                                }
                    ...
                    case 'ready':
                        puppet.on('ready', () => {
                            log.silly(
                                'WechatyPuppetMixin',
                                '__setupPuppetEvents() puppet.on(ready)'
                            )
                    ...
                    case 'room-invite':
                        puppet.on('room-invite', async (payload) => {
                            const roomInvitation = this.RoomInvitation.load(
                                payload.roomInvitationId
                            )
                    ...
                    case 'room-join':
                        puppet.on('room-join', async (payload) => {
                            try {
                                ...
                                const inviteeList = inviteeListAll.filter(
                                    (c) => !!c
                                ) as ContactInterface[]
                    ...
                    case 'room-leave':
                        puppet.on('room-leave', async (payload) => {
                            try {
                                ...
                                const leaverList = leaverListAll.filter(
                                    (c) => !!c
                                ) as ContactInterface[]
                            ...
                            try {
                                switch (payloadType) {
                                    ...
                                    case PUPPET.types.Payload.Contact: {
                                        const contact = (await this.Contact.find({
                                            id: payloadId
                                        })) as unknown as undefined | ContactImpl
                                    ...
                                    case PUPPET.types.Payload.Room: {
                                        const room = (await this.Room.find({
                                            id: payloadId
                                        })) as unknown as undefined | RoomImpl
