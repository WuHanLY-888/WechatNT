...
export class Io {
    ...
    private initEventHook() {
        ...
        wechaty.on('login', (user) =>
            this.send({ name: 'login', payload: wechaty.puppet.contactPayload(user.id) })
        )
        wechaty.on('logout', (user) =>
            this.send({ name: 'logout', payload: wechaty.puppet.contactPayload(user.id) })
        )
    ...
    private wsOnOpen(ws: WebSocket): void {
        ...
        const name = 'sys'
        const payload =
            'Wechaty version ' + this.options.wechaty.version() + ` with CUID: ${this.id}`
